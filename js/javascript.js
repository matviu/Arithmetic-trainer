(function(){
  ...
  function startArithmTrainer() {
    ...
    btnProceed.onclick = function() {
      ...
      //данный обработчик не работает, т.к. addEventListener не перезатирает себя самого на предыдущем уровне рекурсии, и с каждым вызовом addEventListener'а становиться на один обработчик больше и arithmTrainerProceed вызывается по несколько раз.
